...
int mc_count;
	...
	if (net->flags & IFF_PROMISC) {
		rx_ctl |= 0x02;
	} else if (net->flags & IFF_ALLMULTI || mc_count > QF_MCAST_MAX) {
		rx_ctl |= 0x04;
#if LINUX_VERSION_CODE < KERNEL_VERSION(2, 6, 35)
        } else if (mc_count == 0) {
#else
...
#if (LINUX_VERSION_CODE < KERNEL_VERSION(2,6,29))
// backport of eth_mac_addr
// http://www.spinics.net/lists/linux-wireless/msg44824.html
...
static int qf9700_bind(struct usbnet *dev, struct usb_interface *intf)
{
	...
	dev->net->set_mac_address = eth_mac_addr;
#else
